        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-8 md:gap-12 mb-12 md:mb-20">
          <div className="space-y-4 md:space-y-6">
            <motion.div 
              ...
              className="inline-flex items-center gap-3 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-primary/10 text-primary text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] border border-primary/10 shadow-sm"
            >
            ...
            <h1 className="text-4xl md:text-8xl font-black tracking-tighter leading-[0.85] font-display">
              აქტიური <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary/60">ტვირთები</span>
            </h1>
            <p className="text-muted-foreground text-lg md:text-2xl font-medium leading-relaxed max-w-md">
              იპოვეთ საუკეთესო შემოთავაზებები ტვირთების გადასაზიდად მთელი საქართველოს მასშტაბით.
            </p>
          </div>
          
          <div className="flex items-center gap-3 md:gap-4 w-full lg:w-auto">
            <div className="relative flex-1 lg:w-96 group">
              <Search className="absolute left-5 md:left-6 top-1/2 -translate-y-1/2 h-4 w-4 md:h-5 md:w-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
              <input 
                placeholder="ძებნა..." 
                className="w-full pl-12 md:pl-16 h-14 md:h-16 rounded-2xl md:rounded-[2rem] border-none bg-card/50 backdrop-blur-sm shadow-2xl shadow-black/5 focus-visible:ring-2 focus-visible:ring-primary/20 transition-all text-sm md:text-base font-bold outline-none"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            <Button variant="outline" size="icon" className="h-14 w-14 md:h-16 md:w-16 rounded-2xl md:rounded-[2rem] border-none bg-card/50 backdrop-blur-sm shadow-2xl shadow-black/5 hover:bg-secondary shrink-0 transition-all active:scale-95">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredPosts.map((post, index) => (
                  <motion.div
                    ...
                    layout
                  >
                    <Card className="group relative h-full rounded-[2rem] md:rounded-[3rem] border-none shadow-sm hover:shadow-[0_40px_80px_rgba(0,0,0,0.1)] transition-all duration-700 overflow-hidden bg-card/50 backdrop-blur-sm flex flex-col border border-border/50">
                      <div className="p-6 md:p-10 pb-4 md:pb-6 space-y-6 md:space-y-10 flex-1">
                        <div className="flex items-center justify-between">
                          <div className="px-3 py-1 md:px-4 md:py-1.5 rounded-full bg-primary/10 text-primary text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] border border-primary/10">
                            ტვირთი
                          </div>
                      ...
                      <div className="p-6 md:p-10 pt-4 md:pt-6 border-t border-border/50 bg-secondary/20">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 md:gap-6">
                          <div className="space-y-1 md:space-y-1.5">
                            <p className="text-[8px] md:text-[9px] font-black text-muted-foreground uppercase tracking-widest opacity-60">ბიუჯეტი</p>
                            <p className="text-3xl md:text-4xl font-black text-primary tracking-tighter leading-none">₾{post.price}</p>
                          </div>
                          ...
                          <div className="flex items-center gap-2 md:gap-3 w-full sm:w-auto">
                            <Button 
                              variant="ghost" 
                              size="icon" 
                              className="h-12 w-12 md:h-14 md:w-14 rounded-xl md:rounded-2xl bg-card hover:bg-primary/10 hover:text-primary transition-all active:scale-90 shadow-sm border border-border/50 shrink-0"
                            ...
                            <Button 
                              ...
                              className="flex-1 sm:flex-none h-12 md:h-14 rounded-xl md:rounded-2xl px-6 md:px-8 font-black text-[9px] md:text-[10px] uppercase tracking-widest group transition-all active:scale-95 shadow-xl shadow-primary/20"
                            >
                              მიწერა
